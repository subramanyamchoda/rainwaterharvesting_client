...
const Exp = () => {
  ...
  const [polarData, setPolarData] = useState({
    labels: ['Water Level', 'Water Flow', 'Electricity Generated', 'Battery Consumption'],
    datasets: [{
  ...
  useEffect(() => {
    updatePolarChart();
  }, [data]); // Updates the Polar Area chart whenever `data` changes
  ...
  const fetchPredictionData = async () => {
    try {
      ...
      setData(prevData => ({
        ...
        batteryConsumption: [...prevData.batteryConsumption, predictedData.batteryConsumption]
      }));
    } catch (error) {
    ...
    setPolarData({
      ...
      datasets: [{
        data: [
          ...
          data.batteryConsumption.length > 0 ? data.batteryConsumption[data.batteryConsumption.length - 1] : 0
        ],
        backgroundColor: [
          'rgba(0,123,255,0.6)',  // Water Level - Blue
          'rgb(255, 99, 132)',  // Water Flow - Red
            // Turbine Speed - Light Blue
          'rgba(255,206,86,0.6)',  // Electricity Generated - Yellow
          'rgba(153,102,255,0.6)'  // Battery Consumption - Purple
        ],
        borderColor: [
          'rgba(0,123,255,1)',
          'rgba(255,99,132,1)',  // Ensure Red Border for Water Flow
          'rgba(54,162,235,1)',
          'rgba(255,206,86,1)',
          'rgba(153,102,255,1)'
        ],
    ...
    <div className="w-full min-h-screen bg-gradient-to-b from-blue-200 to-gray-200 flex flex-col animate-fade-in">
      ...
      <div className="w-full flex-1 overflow-y-auto p-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {['waterLevel', 'waterFlow', 'turbineSpeed', 'electricityGenerated', 'batteryConsumption'].map((key, index) => (
  ...
  <h1 className="text-lg font-bold text-center text-indigo-600">📊 Overall Data Statistics</h1>
  <div className="h-full">
    <PolarArea data={polarData} options={{
      responsive: true,
      maintainAspectRatio: false, // Ensures it doesn't stretch
      plugins: {
        legend: {
          position: 'top', // Optional, you can adjust the legend's position
        },
      },
      scales: {
        r: {
          min: 0, // You can adjust the min and max of the polar chart to fit your data
          max: 30, // Adjust based on the range of your data
        },
      },
    }} />
  </div>
      ...
      <div className=" bg-blue-600 text-white text-lg font-semibold w-full text-center flex flex-row items-center justify-center py-6  space-x-16">
